...
public class Line {
    // 사다리 한 칸
    private List<Block> lungs;

    public Line(int userNum) {
        this.lungs = generateBlocks(userNum);
    }

    private List<Block> generateBlocks (int userNum) {
        List<Boolean> booleanList = RandomBoolean.generate(userNum - 1);
        return booleanList.stream().map(Block::of).collect(Collectors.toList());
    }
    ...
    public boolean getValidLung (int index) {
        return lungs.get(index) == Block.BRIDGE;
    }
}
